...
object SessionHelper {
    ...
    suspend fun getLatestAttendanceReport(studentId: String): Map<String, Any>? =
        withContext(Dispatchers.IO) {
            ...
            try {
                val roomsSnapshot = roomsRef.get().await()
                var latestReport: Map<String, Any>? = null
                var latestTimestamp: Long = 0

                for (roomSnap in roomsSnapshot.children) {
                    val roomId = roomSnap.key ?: continue
                    val roomName = roomSnap.child("name").getValue(String::class.java) ?: "Unknown Room"
                    val sessionsSnap = roomSnap.child("sessions")

                    for (sessionSnap in sessionsSnap.children) {
                        val sessionId = sessionSnap.key ?: continue
                        val sessionStatus =
                            sessionSnap.child("sessionStatus").getValue(String::class.java) ?: "Unknown"
                        val subject = sessionSnap.child("subject").getValue(String::class.java) ?: "Unknown Subject"
                        val teacherId = sessionSnap.child("teacherId").getValue(String::class.java) ?: ""
                        val startTime = sessionSnap.child("startTime").getValue(String::class.java) ?: "N/A"
                        val endTime = sessionSnap.child("endTime").getValue(String::class.java) ?: "N/A"

                        val attendanceSnap = sessionSnap
                            .child("attendance")
                            .child(currentDate)
                            .child(studentId)

                        if (attendanceSnap.exists()) {
                            val timeScanned = attendanceSnap.child("timeScanned").getValue(String::class.java) ?: "Unknown"
                            val status = attendanceSnap.child("status").getValue(String::class.java) ?: "Unknown"
                            val confidence = attendanceSnap.child("confidence").getValue(String::class.java) ?: "N/A"
                            val lateDuration = attendanceSnap.child("lateDuration").getValue(Int::class.java) ?: 0
                            val recordTime = attendanceSnap.child("timestamp").getValue(Long::class.java)
                            ...
                            if (recordTime > latestTimestamp) {
                                ...
                                latestReport = mapOf(
                                    ...
                                    "timeScanned" to timeScanned
                                )
                            }
                        }
                    }
                }
                return@withContext latestReport
            } catch (e: Exception) {
                Log.e(TAG, "❌ Error fetching attendance report: ${e.message}", e)
